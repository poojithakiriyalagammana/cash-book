...
CREATE TABLE transaction_types(
  ...
  Future _upgradeDB(Database db, int oldVersion, int newVersion) async {
    ...
    if (oldVersion < 3) {
      // Rename party_name to transaction_type_name in transactions table
      await db.execute('''
    ...
    if (oldVersion < 4) {
      // Add the 'note' column to the transactions table
      await db.execute('''
    ...
    if (oldVersion < 7) {
      ...
      // Modify existing tables to include book_id
    ...
    if (oldVersion < 8) {
      ...
      // Update existing default transaction types with book ID
  ...
  Future<int?> getCurrentBookId() async {
    // If no book is set, get the first book or create one
    if (_currentBookId == null) {
      final books = await getAllBooks();
      if (books.isEmpty) {
        // Create first book if none exists
...
/*----------------------*/
  ...
  ''', [bookId]);
    ...
    // Update the book's total balance
  ...
  Future<RecurringTransaction?> getRecurringTransactionByTransactionProperties(
    ...
    // Match based on similar properties between transaction and recurring transaction
    final result = await db.query(
      'recurring_transactions',
      where:
...
// Updated delete method to use properties instead of ID
